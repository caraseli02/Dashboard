<template>
  <div v-if="getUser && userData[0]">
    <transition enter-class="opacity-0" enter-active-class="ease-out transition-medium" enter-to-class="opacity-100"
      ...
      <div @keydown.esc="toggleSidebar" v-show="showSidebar" class="z-10 fixed inset-0 transition-opacity">
        <div @click="toggleSidebar" class="absolute inset-0 bg-black opacity-50" tabindex="0"></div>
      </div>
    </transition>
    <aside v-if="userData"
      ...
      <div class="overflow-hidden shadow max-w-xs">
        <div class="bg-gradient-to-r from-green-400 to-blue-500 w-full h-16 rounden-b-lg"></div>
        ...
        <div class="text-center px-3 pb-6 pt-2">
          ...
          <button class="flex mt-2" v-else>
            <input v-model="surname"
              ...
              id="surname" type="text" placeholder="Apellidos" />
            <span @click="addSurname(userData[0])" class="p-2 w-10 bg-green-500 rounded">OK</span>
          </button>
      ...
      <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0 mt-4">
        <label class="flex justify-start items-center uppercase tracking-wide text-secondary text-xs font-bold mb-2" for="grid-state">
          Horario de Trabajo
          <svg class="mx-2 text-blue-500" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
        ...
        <div class="flex border-b-2 border-gray-500 items-center text-primary p-2">
          <div class="flex">
            <span v-if="'schedule' in userData[0]" class="block font-bold text-primary mt-2 text-xl">{{
              userData[0]["schedule"] }} horas</span>
            <span v-if="userData[0].eatHour" class="text-xs font-bold ml-8 flex flex-col justify-center items-center"><i
                class="gg-coffee text-xl"></i> 1 hora</span>
          </div>
        </div>
      </div>
      <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0 mt-4">
        ...
        <div class="flex items-center p-2 border-b-2 border-gray-500">
          <div>
            ...
            <span v-if="'workplace' in userData[0]" class="block text-xl font-bold text-primary">{{
              userData[0]["workplace"] }}</span>
          </div>
        </div>
      </div>
      <div class="fixed bottom-0 w-full">
        <button @click="closeSession" class="flex items-center p-4 text-white bg-blue-500 hover:bg-blue-600 w-full">
