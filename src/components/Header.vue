    ...
    <div class="mx-auto flex justify-center items-center">
      <nav class="flex items-center justify-between md:justify-between w-full">
        <section class="flex justify-between items-center p-3">
          <button v-if="!loadingMap" class="ml-2 flex justify-center items-center p-2" @click="showMapAction" :class="
          ...
          <span v-if="showMap" @click="showMapAction" class="text-center text-gray-900 
          bg-gradient-to-r from-red-400 via-pink-500 to-red-500 
          font-bold rounded-r-lg p-2">
            x
          </span>
          <router-link to="/" class="ml-2 flex justify-center items-center p-2 rounded-lg glass-gray"
            v-if="!user && $route.path !== '/'">Inicio</router-link>
          <router-link to="sign-in" class="ml-2 flex justify-center items-center p-2 rounded-lg glass-gray"
            v-if="!user && $route.path !== '/sign-in'">Entrar</router-link>
          <router-link to="sign-up" class="ml-2 flex justify-center items-center p-2 rounded-lg glass-gray"
            v-if="!user && $route.path !== '/sign-up'">Registrate</router-link>
          <!-- Side Bar -->
        </section>
        <div v-if="user" @click="toggleSidebar" class="flex items-center">
          <button class="flex justify-center items-center p-3 rounded-lg"
            :class="showSidebar ? 'bg-gray-300':'glass-gray'" aria-label="Open Menu">
            <icon-base icon-name="profile">
              <icon-profile />
            </icon-base>
          </button>
        </div>
        <button @click="closeSession" class="flex justify-center items-center p-3 rounded-lg glass-gray mr-2" v-if="user">
          Salir
