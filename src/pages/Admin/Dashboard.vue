<template>
  <div class="flex flex-col h-full">
    <div class="w-full px-4 lg:p-10">
      <monthSelector v-if="userData.length > 0" :workplace="userData[0].workplace" :selectedWorkplace="workplace"
        :getAsistFunc="true" />
      <section v-if="users !== null && users.length > 1"
      ...
      <section class="flex md:mt-4">
        <div @click="showSidebar = !showSidebar"
          ...
          <!-- Please refer: https://github.com/shubhamjain/svg-loader -->
          <svg width="42px" height="42px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
            class="rounded-lg w-16 p-2 text-primary" :class="!showSidebar ? 'bg-blue-800' : ''">
            <path
        ...
        <div v-if="!selectedUser"
          ...
          <span @click="
            ...
            :class="!showMonthAttends ? 'bg-blue-800' : ''">{{ filtredAttendsDay ? filtredAttendsDay.getDate() : "Hoy"
            }}</span>
          <span @click="(filtredAttendsDay = null), (showMonthAttends = true)"
            class="w-full h-full rounded-xl flex justify-center items-center text-primary font-semibold text-xl"
            :class="showMonthAttends ? 'bg-blue-800' : ''">Mes</span>
        </div>
      </section>
      <div class="overflow-hidden lg:overflow-visible pb-24 lg:pb-0">
        <div class="transform lg:transform-none transition-all duration-300"
          :class="showSidebar ? '-translate-x-full' : ''">
          <div class="flex mt-4 lg:mt-10 lg:space-x-10">
            <div class="w-full flex-shrink-0 lg:w-2/3 lg:flex-shrink rounded-xl">
              <div v-if="filtredAttends.length > 0" class="flex space-x-4 lg:space-x-10">
                <infoCard v-if="!selectedUser">
                  ...
                  <template v-slot:name><span
                      class="block uppercase text-sm tracking-wider text-secondary">Operarios</span></template>
                  <template v-slot:number><span class="text-2xl lg:text-2xl font-semibold text-primary">{{
                      workplaceUsers.length }}</span></template>
                </infoCard>
                <infoCard>
                  <template v-slot:svg>
                    <svg width="32px" height="32px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                      fill="currentColor" class="text-green-500 mx-auto">
                      <path fill-rule="evenodd"
                  ...
                  <template v-slot:name><span
                      class="block uppercase text-sm tracking-wider text-secondary">Entradas</span></template>
                  <template v-slot:number><span class="text-2xl lg:text-2xl font-semibold text-primary">{{ punchInCount
                      }}</span></template>
                </infoCard>
                <infoCard>
                  <template v-slot:svg>
                    <svg width="32px" height="32px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                      fill="currentColor" class="text-red-500 mx-auto">
                      <path fill-rule="evenodd"
                  ...
                  <template v-slot:name><span
                      class="block uppercase text-sm tracking-wider text-secondary">Salidas</span></template>
                  <template v-slot:number><span class="text-2xl lg:text-2xl font-semibold text-primary">{{ punchOutCount
                      }}</span></template>
                </infoCard>
              </div>
              <!-- Attend Day Selector -->
              <attendDaySelector v-if="!selectedUser" :workedDaysList="workedDaysList" :theme="theme"
                :filtredAttendsDay="filtredAttendsDay" v-on:sendDay="filtredAttendsDay = $event" />
              <!-- Attend Rows -->
              <transition-group class="grid grid-flow-col auto-cols-max py-2 overflow-y-scroll mt-2" name="list"
                tag="section">
              ...
              <transition name="fade" mode="out-in">
                <div class="w-full h-full z-10 absolute top-0 bg-gray-800 dark:bg-gray-600 bg-opacity-75"
                  ...
                  <attendChange v-on:dataChanged="
                      (dataToChange = null),
                        (gpsData = null),
                        (adminChange = true)
                    " :dataToChange="dataToChange" :gpsData="gpsData" />
                  <button
            ...
            <div class="w-full h-64 flex-shrink-0 lg:w-1/3 lg:flex-shrink rounded-xl">
              <gMaps v-if="filtredAttends.length > 0" height="500" :markers="markers" />
            </div>
          </div>
        </div>
      </div>
