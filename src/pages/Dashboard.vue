<template>
  <article v-if="geolocation.lat && geolocation.lng">
    ...
    <section v-if="today" class="grid grid-flow-row auto-rows-max gap-4">
      <ul v-if="checkCalendarToday === true || checkCalendarToday === selectedMes"
        ...
        <li class="flex justify-center items-center mb-2">
          <label class="text-xl mb-2 text-gray-200" for="vol">Temperatura :</label>
          <select v-model="temperature" id="vol" name="vol" required
        ...
        <li class="mx-auto relative text-lg flex justify-center items-center p-2">
          <drag-verify v-on:passcallback="swipeHandler()" class="" :width="swiper.width" :height="swiper.height"
            :text="swiper.text" :success-text="swiper.successText" :background="swiper.background"
      ...
      <ul v-for="(attend, index) in attendList" :key="index"
        ...
        <li class="w-24 mx-auto flex flex-col justify-around text-lg items-center p-4">
          Entrada
          <span class="text-2xl text-green-800">
            &#8595;{{ attend.data.enterTime.slice(11, 16) }}</span>
        </li>
        <li v-if="attend.data.leaveTime" class="w-20 mx-auto flex flex-col justify-around text-lg items-center p-4">
          Salida
          <span class="text-2xl text-red-800">
            &#8593;{{ attend.data.leaveTime.slice(11, 16) }}</span>
        </li>
        <li class="flex flex-col justify-center text-red-900 font-bold items-center h-full w-full z-50"
          v-if="'curentTime' in attendList[0]">
          <button v-if="actualMonthCheck && !attend.data.leaveTime && attendIsToday" @click="saveLeaveTime(attend)"
          ...
          <div v-if="!attend.msgLeave && !attend.data.leaveTime && !attendIsToday">
            No tiene Salida
            <button
      ...
      <ul v-if="workedTime && actualMonthCheck" class="glass-light w-screen h-24 grid grid-flow-col auto-cols-max">
        <li class="w-full mx-auto flex flex-col justify-around text-lg items-center p-4">
          Has trabajado
          <span class="text-xl text-green-800"> {{ workedTime }}</span>
        </li>
        <li class="w-full mx-auto flex flex-col justify-around text-lg items-center p-4" v-if="extraHors">
          Horas Extra
