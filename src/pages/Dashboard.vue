  ...
  <article v-if="geolocation.lat && geolocation.lng">
    ...
    <section v-if="today" class="grid grid-flow-row auto-rows-max gap-4">
      <ul v-if="checkCalendarToday === true || checkCalendarToday === selectedMes"
        ...
        <li class="mx-auto relative text-lg flex flex-col justify-center items-center p-2">
          <span class="text-primary">Desliza a la Derecha</span>
          <drag-verify v-on:passcallback="swipeHandler()" class="border-2 border-green-300 shadow-lg"
            :width="swiper.width" :height="swiper.height" :text="swiper.text" :success-text="swiper.successText"
            :background="swiper.background" :progress-bar-bg="swiper.progressBarBg" :completed-bg="swiper.completedBg"
            :handler-bg="swiper.handlerBg" :handler-icon="swiper.handlerIcon" :text-size="swiper.textSize"
            :success-icon="swiper.successIcon" :circle="swiper.isCircle">
          </drag-verify>
        </li>
      </ul>
      ...
      <ul v-for="(attend, index) in attendList" :key="index"
        ...
        <li class="flex flex-col justify-center text-red-900 font-bold items-center h-full"
      ...
      <ul v-if="workedTime && actualMonthCheck"
        :class="`glass-${theme} w-screen mx-auto h-24 flex justify-center items-center`">
        <li class="w-full mx-auto flex justify-around text-lg items-center p-4 text-secondary">
          <i class="gg-time mb-2 text-green-800 dark:text-green-500"></i>
          <div class="flex flex-col justify-around">
            Has trabajado
            <span class="text-2xl text-green-800 dark:text-green-500"> {{ workedTime }}</span>
          </div>
        </li>
        <li class="w-full text-secondary mx-auto flex justify-around text-lg items-center p-4"
          v-if="extraHors">
          <i class="gg-insert-after text-red-800 dark:text-red-500 mb-2 "></i>
          <div class="flex flex-col justify-around">
            Horas Extra
            <span class="text-2xl text-red-800 dark:text-red-500">{{ extraHors }} </span>
          </div>
        </li>
      </ul>
    ...
    methods: {
        ...
        if (this.actualMonthCheck) {
          ...
          if (value) {
            this.$confirm(`Apuntar Salida?`).then(() => {
              ...
              value["data"]["leaveTime"] = this.getRoundedDate(5);
      ...
      async swipeHandler() {
        ...
        this.setAttendance({
          ...
          dttm: new Date(),
          enterTime: this.getRoundedDate(5),
