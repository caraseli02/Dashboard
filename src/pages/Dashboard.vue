<template>
  <article v-if="geolocation.lat && geolocation.lng"
    ...
    <div class="mb-8 flex justify-between items-center w-full text-lg">
      <button class="glass-gray" :class="enterModal ? 'shadow-lg bg-gray-300' : ''"
        @click="(enterModal = true), (leaveTime = null)">
        Entrada
      </button>
      <button v-if="userData.enterTime" class="glass-gray" :class="!enterModal ? 'shadow-lg bg-gray-300' : ''"
    ...
    <div class="w-full p-2 mx-auto glass-light z-10">
      ...
      <div class="grid grid-cols-1 gap-2 p-2 content-center">
        ...
        <div v-if="enterModal" class="flex justify-between flex-col items-center">
          <div class="flex justify-center items-center">
            <datetime :input-class="
              ...
              " type="datetime" v-model="userData.enterTime" value-zone="local" zone="local"
              placeholder="Seleccionar la fecha"></datetime>
            <span v-if="userData.enterTime" @click="userData.enterTime = null" class="mb-4 text-center text-gray-900 
            bg-gradient-to-r from-red-400 via-pink-500 to-red-500 
            font-bold p-2 rounded-r-lg px-4 text-lg">
              x
            </span>
          </div>

          <p class="flex flex-col justify-center items-center mb-2">
            <label class="text-xl mb-2" for="vol">Temperatura : {{ userData.temperature }}</label>
            <input v-model="userData.temperature" type="range" id="vol" name="vol" min="35" max="40" step="0.1" />
          </p>
        </div>
        <!-- leaveHours  Modal -->
        <div v-if="userData.enterTime && !enterModal" class="flex justify-between flex-col items-center">
          <div class="flex justify-center items-center">
            <datetime type="datetime" value-zone="local" zone="local" :input-class="
                leaveTime
                  ? 'mb-4 mx-auto enterTime p-2 rounded-l-lg text-green-500 font-bold text-center text-lg shadow-xl'
                  : 'mb-4 mx-auto enterTime p-2 rounded-lg text-green-500 font-bold text-center text-lg  animate-bounce'
              " placeholder="Pulsa para seleccionar" v-model="leaveTime" :min-datetime="userData.enterTime"></datetime>
            <span v-if="leaveTime" @click="leaveTime = null" class="mb-4 text-center text-gray-900 
          bg-gradient-to-r from-red-400 via-pink-500 to-red-500 
          font-bold p-2 rounded-r-lg px-4 text-lg">
              X
            </span>
    ...
    <section v-if="userData.enterTime" class="w-full  mt-4 text-center flex relative z-0">
      <drag-verify v-if="!dateExist[0]" v-on:passcallback="swipeHandler()" class="mt-8" :width="swiper.width"
        :height="swiper.height" :text="swiper.text" :success-text="swiper.successText" :background="swiper.background"
        :progress-bar-bg="swiper.progressBarBg" :completed-bg="swiper.completedBg" :handler-bg="swiper.handlerBg"
        :handler-icon="swiper.handlerIcon" :text-size="swiper.textSize" :success-icon="swiper.successIcon"
        :circle="swiper.isCircle">
      </drag-verify>
      <div class="w-full text-xl text-gray-100 text-left font-bold glass-light"
        ...
        <button @click="
            deleteAsist(dateExist[0].id),
              (leaveTime = null),
              (userData.enterTime = null),
              (enterModal = true)
          "
      ...
      <div class="w-full mx-auto text-left mt-4 rounded-xl p-2 glass-light" v-else>
        <span class=" text-md text-gray-800 mb-4">Info sobre la fecha seleccionada:</span>
        <div class="flex justify-around item">
          <ul
          ...
          <ul
            ...
            <li>
              <button v-if="enterModal" @click="enterModal = false">
                <span class="bg-green-500 hover:bg-green-400 text-sm text-white py-2 px-4 rounded
                " v-if="enterModal">Apuntar</span>
              </button>
            </li>
          </ul>
        </div>
        <div class="flex justify-around items-center mt-2">
          <button class="my-3" v-if="leaveTime" @click="enterModal = false">
            <span class="bg-gradient-to-r from-teal-400 via-green-500 to-blue-500 hover:bg-green-400 text-white py-2 px-4 text-lg rounded
            " @click="saveLeaveTime(dateExist)">Confirmar</span>
          </button>
        </div>
      </div>
    </section>
  ...
  <section v-else class="flex flex-col justify-center items-center ">
    ...
    <div v-else>
      ...
      <a class="bg-gradient-to-r from-blue-600 via-teal-500 to-blue-700 hover:bg-green-400 text-white py-2 px-4 text-lg rounded"
        href="https://support.google.com/accounts/answer/3467281?hl=es-419">
        Como administrar tu ubicación
        <span class="block mt-2 text-sm text-center">( Ayuda de Cuenta de Google )</span></a>
    </div>
  </section>
</template>
